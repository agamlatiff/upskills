                                    ...
                                    <div className="border border-slate-800 rounded-lg overflow-hidden">
                                        {course.curriculum.map((section, index) => (
                                            <div key={index} className="border-b border-slate-800 last:border-b-0">
                                                <button onClick={() => toggleSection(index)} className="w-full flex justify-between items-center p-4 bg-slate-800/50 hover:bg-slate-800 transition-colors">
                                                    <span className="font-bold text-white text-left">{section.title}</span>
                                                    <div className="flex items-center text-sm text-slate-400">
                                                        <span>{section.lessons.length} lectures</span>
                                                        {openSections[index] ? <ChevronUpIcon className="h-5 w-5 ml-4" /> : <ChevronDownIcon className="h-5 w-5 ml-4" />}
                                                    </div>
                                                </button>
                                                {openSections[index] && (
                                                    <ul className="p-4 bg-brand-dark">
                                                        {section.lessons.map((lesson, lessonIndex) => (
                                                            <li key={lessonIndex} className="flex items-center py-2 text-slate-300">
                                                                <VideoCameraIcon className="h-5 w-5 text-slate-500 mr-3 flex-shrink-0" />
                                ...
                                <div className="bg-brand-dark border border-slate-800 rounded-2xl overflow-hidden shadow-lg">
                                    ...
                                    <div className="p-6">
                                        <div className="mb-4 text-center">
                                            <h2 className="text-2xl font-bold text-white">Part of Upskill Pro</h2>
                                            <p className="text-slate-400 mt-2">Unlock this course and 50+ others with a subscription.</p>
                                        </div>
                                        <div className="space-y-3">
                                            <Link to={`/courses/${course.slug}/learn`} className="w-full block text-center py-3 bg-blue-600 text-white font-semibold rounded-full hover:bg-blue-700 transition-colors">
                                                Start Learning Now
                                            </Link>
                                            <button
                                            ...
                                            >
                                                <HeartIcon className={`h-5 w-5 mr-2 transition-colors ${wishlisted ? 'text-red-500 fill-red-500' : ''}`} />
                                                {wishlisted ? 'Saved to Wishlist' : 'Add to Wishlist'}
                                            </button>
                                        </div>
                                        
                                        <div className="mt-6">
                                            <h3 className="font-bold text-white mb-3">Your subscription includes:</h3>
                                            <ul className="space-y-2 text-sm">
                                                <li className="flex items-center text-slate-300"><VideoCameraIcon className="h-5 w-5 mr-3 text-slate-400" />{totalLessons} on-demand lectures</li>
                                                <li className="flex items-center text-slate-300"><FileTextIcon className="h-5 w-5 mr-3 text-slate-400" />Articles & resources</li>
                                                <li className="flex items-center text-slate-300"><MobileIcon className="h-5 w-5 mr-3 text-slate-400" />Access on mobile and TV</li>
                                                <li className="flex items-center text-slate-300"><TrophyIcon className="h-5 w-5 mr-3 text-slate-400" />Certificate of completion</li>
                                            </ul>
                                        </div>
                                    </div>
                                </div>
